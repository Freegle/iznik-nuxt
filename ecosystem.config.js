module.exports = {
  ...
  deploy: {
    ...
    production: {
      ...
      'post-deploy':
        'monit stop nginx && scp -r app4:/var/build/iznik-nuxt/* . && npx patch-package && cp restartfd /etc && chmod +x /etc/restartfd && cp waitfornode /etc && chmod +x /etc/waitfornode && pm2 restart FD-production --update-env && /etc/waitfornode && monit start nginx'
    },
    ...
    development: {
      ...
      'post-deploy':
        'scp -r app4:/var/build/iznik-nuxt/* . && npx patch-package && cp restartfd /etc && chmod +x /etc/restartfd && cp waitfornode /etc && chmod +x /etc/waitfornode && pm2 restart FD-development --update-env && /etc/waitfornode'
    },
    ...
    debug: {
      ...
      'pre-setup': 'echo `hostname` && git checkout -- package-lock.json && git checkout -- static/sw.js && git checkout -- nuxt.config.js',
      'post-deploy':
        'scp -r app4:/var/build/iznik-nuxt/* . && npx patch-package && cp restartfd /etc && chmod +x /etc/restartfd && cp waitfornode /etc && chmod +x /etc/waitfornode && pm2 restart FD-debug --update-env && /etc/waitfornode'
    }
  }
}
