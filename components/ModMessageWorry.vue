    ...
    <NoticeMessage v-for="word in message.worry" :key="'worry-' + message.id + '-' + word.worryword.id" variant="warning" class="mb-1">
      ...
      <div v-if="expand">
        ...
        <p v-if="word.worryword.type === 'Medicine'">
          This post looks as though it might contain a drug, medicine or supplement.  These are not legal on Freegle.  Please do not approve this without checking on <a href="https://discourse.ilovefreegle.org/" data-realurl="true" target="_blank" rel="noopener noreferrer">Central</a> first.
        </p>
        <p>You can find more information <a href="http://wiki.ilovefreegle.org/Worry_Words" data-realurl="true" target="_blank" rel="noopener noreferrer">here</a>.</p>
      </div>
    </NoticeMessage>
  </div>
</template>
